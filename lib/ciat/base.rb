...
module CIAT
  ...
  class Base
    ...
    def self.run_tests_on_files(filenames, compiler, executor) #:nodoc:
      filenames.map { |filename| run_test(filename, compiler, executor) }      
    end
    
    def self.run_test(filename, compiler, executor) #:nodoc:
      CIAT::Test.new(Filenames.new(filename.gsub(/\.txt$/, '')), compiler, executor).run_test
    end
    
    def self.write_file(filename, content) #:nodoc:
      File.open(filename, "w") do |file|
        file.write content
      end
    end
    
    def self.generate_html(test_reports) #:nodoc:
