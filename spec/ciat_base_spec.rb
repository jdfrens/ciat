...
describe CIAT::Base, "top level test function" do
  ...
  it "should get a directory listing " do
    ...
    CIAT::Base.should_receive(:run_tests_on_files).with(filenames, @compiler).and_return(tests)
    ...
    CIAT::Base.run_tests(@compiler).should == result
  end
  
  it "should run tests on files" do
    filenames = [mock("filename1"), mock("filename2")]
    results = [mock("result1"), mock("result2")]
    CIAT::Base.should_receive(:run_test).with(filenames[0], @compiler).and_return(results[0])
    CIAT::Base.should_receive(:run_test).with(filenames[1], @compiler).and_return(results[1])
    
    CIAT::Base.run_tests_on_files(filenames, @compiler).should == results
  end
  ...
  it "should run a test" do
    ...
    CIAT::Test.should_receive(:new).with(filenames, @compiler).and_return(test)
    test.should_receive(:run_test).and_return(result)
    
    CIAT::Base.run_test("foo.txt", @compiler).should == result
  end
